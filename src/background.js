...
const isLocalUrl = (url) => {
  return String(url).startsWith('chrome://') || String(url).startsWith('chrome-extension://')
}
...
chrome.webRequest.onHeadersReceived.addListener(function (info) {
  ...
  return {
    responseHeaders: headers.filter(header => {
      ...
      return ['x-frame-options', 'content-security-policy'].indexOf(name) === -1
    })
  }
}, {
  urls: ["<all_urls>"],
...
chrome.runtime.onMessage.addListener(
  function (request, sender, sendResponse) {
    if (request.message === "GET_TAB_URL") {
      sendResponse({tabUrl: isLocalUrl(lastOpenedUrl) ? null : lastOpenedUrl});
    }
    return true;
});