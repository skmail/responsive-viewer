...
const useStyles = makeStyles(theme => ({
  root: {
    ...
    [theme.breakpoints.up('sm')]:{
      minHeight: `calc(100vh - ${theme.mixins.toolbar[theme.breakpoints.up('sm')].minHeight}px)`,
    }
  },
  content: props => ({
    display: "flex",
    ...
    <Scrollbars className={classes.root}>
      <div className={classes.content}>
        {visibleScreens.map(screen => (
            <div id={getDomId(screen.id)} className={classes.screen} key={screen.id}>
              <Box display="flex" justifyContent="space-between">
                <FormHelperText className={classes.text}>
                  {screen.name}
                </FormHelperText>
                <FormHelperText className={classes.text}>
                  {screenDirection === 'landscape' ? `${screen.height}x${screen.width}`: `${screen.width}x${screen.height}`}
                </FormHelperText>
              </Box>
              <ScreenIframe
                screen={screen}
