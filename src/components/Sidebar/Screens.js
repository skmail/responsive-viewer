...
const Item = ({value: screen, sortIndex, updateVisibility, classes, onClick, disableSort = false, toggleScreenDialog}) => (
  <Box display="flex" alignItems="center" className={classes.screenRow}>
    <div className={classes.screenRowActions}>
      <IconButton
        ...
        size="small">
        {!screen.visible &&
        <RemoveRedEyeOutlinedIcon className={classes.visibilityIconInactive} fontSize="inherit"/>}
    ...
    <div className={classes.screenDescription}
      ...
      <Typography variant="caption" display={"block"} color="inherit" noWrap className={classes.text}>
        {screen.name}
      </Typography>
      <Typography variant="caption" color="inherit" className={classes.screenSize}>
...
export default (props) => {
  ...
  const {updateVisibility, screens, sortScreens, onClick, toggleScreenDialog} = props
    ...
    <React.Fragment>
      ...
      <Scrollbars style={{height: 300}}>
        <div ref={rootRef} className={classes.screensRoot}>
          ...
          {hasSearch && searchResults.map(screen => (
              <Item
                disableSort={true}
                key={screen.id}
                onClick={onClick}
          ...
          {!hasSearch && (
            <Sortable lockAxis={"y"} helperContainer={() => rootRef.current} onSortEnd={onSortEnd} useDragHandle>
          ...
          {!hasSearch && invisibleScreens.map(screen => (
              <Item
                key={screen.id}
                onClick={onClick}
                classes={classes}
