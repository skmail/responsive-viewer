...
const UserAgentDialog = props => {
  ...
  const {handleSubmit, onSubmit: _onSubmit, invalid, reset, userAgents, open, onClose} = props
  ...
  const uniqueNameValidator = useCallback(
    (value) => {
      const agent = userAgents.find(agent => agent.name.toLowerCase() === value.toLowerCase());
      if(!agent){
        return undefined
      }
        ...
        <DialogContent>
          <div className={classes.body}>
            <TextField
              ...
              label="Name"
              color="secondary"
              validate={[validation.required, uniqueNameValidator]}
              name={"name"}
            />
            ...
            <DialogActions>
              <Button
                ...
                variant={"contained"}
                color={"primary"}
                type={"submit"}>Add User agent</Button>
